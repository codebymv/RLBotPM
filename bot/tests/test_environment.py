...
def test_environment_with_db_data_if_available():
    ...
    assert obs.shape == expected_shape, (
        f"Expected observation shape {expected_shape} but got {obs.shape}"
    )
    assert "capital" in info
    assert info["capital"] == env.initial_capital


def test_sequence_wrapper_shapes():
    """Sequence wrapper should return stacked observations."""
    session = get_db_session()
    try:
        row = session.query(CryptoCandle).first()
        if row is None:
    ...
    base_env = CryptoTradingEnv(dataset=df, interval="1h", max_steps=50)
    seq_env = SequenceStackWrapper(base_env, sequence_length=10, flatten=True)

    obs, info = seq_env.reset()
    assert obs.shape == (10 * base_env.observation_space.shape[0],)
